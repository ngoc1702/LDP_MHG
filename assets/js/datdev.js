...
function itemGallery(id, urlThumb, title) {
  return `<div class="item-gallery relative w-[240px] h-[240px] rounded-3xl rectangle relative shrink-0 cursor-pointer overflow-hidden border-2 border-[#003A34] hover:border-[#D2A97D] hover:underline hover:text-[#D2A97D]"
   style="background-image: url(${urlThumb});" onclick="openLightBox(${id})">
   <div id="title-gallery"
     class="w-full py-4 gap-2.5 absolute bottom-0 bg-[#FFFFFF] rounded-b-3xl text-center text-xl font-semibold leading-7 cursor-pointer hover:underline">
     ${title}
   </div>
 </div>`;
}
